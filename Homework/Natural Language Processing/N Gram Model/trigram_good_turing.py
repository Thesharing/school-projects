...
for key, value in items:
    if value in time3:
        ...
        time3[value] = 1
keySorted = sorted(time3.keys())
for key in keySorted[1:10]:
    time3[key] = (key + 1) * time3[key + 1] / time3[key]
for i in range(11, 100000):
    time3[i] = i
    ...
    for line in ft.readlines():
        ...
        for i in range(0, len(a) - 2):
            if (dict3.get(a[i] + '/' + a[i + 1] + '/' + a[i + 2], 0)) == 0 or dict2.get(a[i] + '/' + a[i + 1], 0) == 0:
                if (dict2.get(a[i + 1] + '/' + a[i + 2], 0) == 0) or (
                            dict1.get(a[i + 2], 0) == 0):
                    l = time1[dict1.get(a[i + 2], 1)] / dict1ALL
                else:
                    l = time2[dict2.get(a[i + 1] + '/' + a[i + 2], 0)] / time1[dict1.get(a[i + 2], 0)]
            else:
                l = time3[dict3.get(a[i] + '/' + a[i + 1] + '/' + a[i + 2], 0)] / time2[dict2.get(a[i] + '/' + a[i + 1], 0)]
            t += math.log(l, 2)
    t = math.pow(2, -t / n)
    print(str(t))
