...
class FPGrowth:
    ...
    def fit(self, transaction_list, min_support: int = 2, min_size: int = 1):
        self.items.clear()
        for itemset, support in find_frequent_itemsets(transaction_list, min_support, True):
            if len(itemset) >= min_size:
                self.items.append((itemset, support))
